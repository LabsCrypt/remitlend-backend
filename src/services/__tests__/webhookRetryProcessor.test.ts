...
import { WEBHOOK_RETRY_CONFIG, getRetryDelayMs } from "../webhookService.js";
...
describe("Webhook Retry Processor", () => {
  ...
  it("caps the backoff delay at the last configured value", () => {
    const maxDelay =
      WEBHOOK_RETRY_CONFIG.RETRY_DELAYS_MS[
        WEBHOOK_RETRY_CONFIG.RETRY_DELAYS_MS.length - 1
      ];
    expect(getRetryDelayMs(WEBHOOK_RETRY_CONFIG.MAX_RETRY_ATTEMPTS)).toBe(
      maxDelay,
    );
    expect(getRetryDelayMs(WEBHOOK_RETRY_CONFIG.MAX_RETRY_ATTEMPTS + 5)).toBe(
      maxDelay,
    );
  });
