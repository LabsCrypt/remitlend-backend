...
export const createWebhookSubscriptionSchema = z.object({
  ...
  eventTypes: z
    .array(z.enum(SUPPORTED_WEBHOOK_EVENT_TYPES as unknown as readonly [string, ...string[]]))
    .min(1, `eventTypes must include at least one of: ${SUPPORTED_WEBHOOK_EVENT_TYPES.join(", ")}`),
...
export const reindexLedgerRangeQuerySchema = z.object({
  fromLedger: z.coerce
    .number()
  ...
  (data) => data.fromLedger <= data.toLedger,
  { message: "fromLedger must be less than or equal to toLedger" },
).refine(
  (data) => data.toLedger - data.fromLedger + 1 <= reindexMaxRange,
